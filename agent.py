...
"""
    ...
    image_keywords = ["generate a picture", "generate an image", "draw", "show me a picture", "create image"]
    if any(k in user_input.lower() for k in image_keywords):
        if resp_text == "TOOL:IMAGE":
            # Use OpenAI image API
            client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
            prompt = user_input  # directly use user input as prompt
            image_response = client.images.generate(
                model="gpt-image-1",
                prompt=prompt,
                size="1024x1024"
            ...
            image_url = image_response.data[0].url
        return f"🖼 Generated image for: '{user_input}'", image_url
